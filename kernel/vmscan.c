...
static ssize_t show_pid_ctl(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "Let's test\n");
}

/* see Documentation/admin-guide/mm/multigen_lru.rst for details */
static ssize_t store_pid_ctl(struct kobject *kobj, struct kobj_attribute *attr,
			     const char *buf, size_t len)
{
	return len;
}

static struct kobj_attribute pid_ctl_attr = __ATTR(
	pid_ctl, 0644, show_pid_ctl, store_pid_ctl
);

static struct attribute *pid_ctl_attrs[] = {
	&pid_ctl_attr.attr,
	NULL,
};

static struct attribute_group pid_ctl_attr_group = {
	.name = "pid_ctl",
	.attrs = pid_ctl_attrs,
};

static int __init init_pid_ctl(void)
{

	if (sysfs_create_group(mm_kobj, &pid_ctl_attr_group))
		pr_err("lru_gen: failed to create sysfs group\n");

	return 0;
};
late_initcall(init_pid_ctl);
